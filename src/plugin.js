// Immersive Art BG — single-layer blur + overlay mask, with button + sliders
export default function () {
  ...
  // ---------- CSS: one background layer w/ ::after mask (the version that worked) ----------
  function ensureStyle() {
    if ($(ID.style)) return;
    const css = `
:root{
  --iab-blur:${cfg.blurPx}px; --iab-dim:${cfg.dim}; --iab-vignette:${cfg.vignette};
}
#${ID.layer}{
...
`;
  ...
  function artFrom(item, size) {
    return item?.attributes?.artwork?.url?.replace?.('{w}x{h}', `${size}x${size}`) || '';
  }
  ...
  // update CSS variables + also set inline filter (belt & suspenders)
  function applyVars() {
    ...
    const el = $(ID.layer);
    if (el) el.style.filter = `blur(${cfg.blurPx}px)`;
  }

  function applyArt() {
    const url = artFrom(mk.nowPlayingItem, 2000);
    setBG(url);
    applyVars();
  }
  ...
  function ensureControls() {
    ...
    if (!btn) {
      ...
      btn.id = ID.btn; btn.title = 'Immersive Art Settings'; btn.textContent = '🎨';
    ...
    let panel = $(ID.panel);
    if (!panel) {
      panel = document.createElement('div'); panel.id = ID.panel; panel.style.display = 'none';
      ['mousedown','click'].forEach(ev => panel.addEventListener(ev, e => e.stopPropagation()));

      const h = document.createElement('h3'); h.textContent = 'Immersive Art BG';
      panel.appendChild(h);
      ...
      resetBtn.onclick = () => {
        ...
        rows.forEach((r,i) => {
          ...
          inp.value = vals[i];
          val.textContent = (i===0 ? `${Math.round(vals[i])}px` : (Math.round(vals[i]*100)/100));
        });
        applyVars();
      };
